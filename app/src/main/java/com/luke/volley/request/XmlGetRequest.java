 ...
 */
public class XmlGetRequest<T, HandlerType extends XmlDefaultHandler<T>> extends PatchedRequest<T> {
	 ...
	 */
	public XmlGetRequest(String url, HandlerType sax_handler, RequestCallback callback) {
		super(Method.GET, url, null, null, callback);
		m_saxParserFactory = SAXParserFactory.newInstance();
		m_xmlHandler = sax_handler;
	}

	public XmlGetRequest(String url, Map<String, String> headers, HandlerType sax_handler, RequestCallback callback) {
		this(url, sax_handler, callback);
		m_headers = headers;
	}
	...
	protected Response<T> parseNetworkResponse(NetworkResponse response) {
		...
		try {
			if (instream != null) {
				...
				inputStreamReader = new InputStreamReader(instream, DEFAULT_NETWORK_ENCODING);
